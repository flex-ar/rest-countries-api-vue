...
const getPropAndConvertToString = func =>
  pipe(saveObj, objectToArray, map(func), x => x.join(', '), defaultString);

const getCapital = pipe(
  Maybe.of,
  map(x => x[0]),
  x => x.get()
);
...
export const normalizer = x => ({
  name: {
    ...
    native: getPropAndConvertToString(getPropInsideArray('common'))(
      x.name.nativeName
    ),
  },
  topLevelDomain: x.tld ? x.tld.join(' ') : '',
  currencies: getPropAndConvertToString(getPropInsideArray('name'))(
    x.currencies
  ),
